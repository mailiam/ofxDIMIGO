...
void boids::setup(float _space, float _sight, float _max_foce){
	...
	for(int i=0; i< NUM_PARTICLES; i++){					//배열 크기 만큼 
		p[i].pos.set(ofRandom(-50,50), ofRandom(-50,50), ofRandom(-50,50));	//파티클의 위치와 속도를 랜덤하게 지정
		p[i].vec.set(ofRandom(-2, 2), ofRandom(-2, 2), ofRandom(-2,2));
		p[i].max_vel = 4;
	}
}
...
void boids::update(){										//각 파티클에 대하여 이동방향 계산 수행
	for(int i=0; i< NUM_PARTICLES; i++){
		...
		p[i].acc = sep*1.5 + ali*1 + coh*1.3;
		
		p[i].pullToCenter(ofPoint(0,0,0));					//중심으로 당기기;
